...
impl MicrophoneOpusRecorder {
    ...
    pub fn new(target_sample_rate: u32, target_channels: u16, _frame_size: usize) -> Result<Self> {
        ...
        // 获取最佳配置
        let (config, actual_sample_rate, actual_channels) = Self::get_optimal_config(&device, target_sample_rate, target_channels)?;

        tracing::info!("🎤 创建录音器: 目标配置({}Hz, {}声道) -> 实际配置({}Hz, {}声道)",
            target_sample_rate, target_channels, actual_sample_rate, actual_channels);
    ...
    /// 获取最佳设备配置
    fn get_optimal_config(device: &Device, target_sample_rate: u32, target_channels: u16) -> Result<(StreamConfig, u32, u16)> {
        ...
        if available_configs.is_empty() {
            return Err(ClientError::AudioError("设备没有可用的音频配置".to_string()));
        }

        // 记录设备支持的配置
        tracing::info!("📊 设备支持的配置:");
        for (min_rate, max_rate, channels, format) in &available_configs {
            tracing::info!("- 采样率: {}-{}Hz, 声道: {}, 格式: {:?}", 
        ...
        tracing::info!("✅ 选择的音频配置: 采样率={}Hz, 声道={}, 缓冲区大小={}", 
            actual_rate, actual_channels, buffer_size);
    ...
    pub fn check_device_status(&self) -> Result<DeviceStatus> {
        ...
        let status = DeviceStatus {
            name: device.name().map_err(|e| ClientError::AudioError(format!("获取设备名称失败: {}", e)))?,
            is_available: true,
    ...
    pub fn start_recording(&mut self) -> Result<mpsc::UnboundedReceiver<Vec<u8>>> {
        ...
        let config = match MicrophoneOpusRecorder::get_optimal_config(&device, self.sample_rate, self.channels) {
        ...
        let stream = match self.create_stream::<f32>(&device, &config, encoder, is_recording.clone(), opus_sender, frame_size, expected_sample_rate) {
            Ok(stream) => stream,
            Err(e) => {
        ...
        f32: cpal::FromSample<T>,
    {
        ...
        // 创建一个线程安全的采样缓冲区
        let sample_buffer = Arc::new(Mutex::new(Vec::with_capacity(frame_size * channels as usize * 2)));
